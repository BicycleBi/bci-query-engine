...
|---|---|---|
| Metadata database | `security_users`, direct/group role assignments, role permissions, `app.artifacts`, and monitoring relations | Security owns identity and grants; Query Engine reads them and is the sole monitoring writer |
...
| Runtime configuration | Expected client key, Bicycle administrator role, and optional client-reporting role | Orchestration owns environment-specific binding and promotion |
...
2. Query Engine verifies that the route client matches the stack's configured
   Analytics client.
3. Query Engine resolves an active, unexpired assignment and an exact report
   permission from the metadata database.
...
6. Usage and Access endpoints return only their bounded semantic responses.

The physical metadata schema does not itself authorize a report. Authorization
requires the configured client boundary, an active identity, a live assignment,
and the exact permission on the canonical Analytics resource.
